# Bootstrappable Quercus Core Builds

This directory contains the files necessary to perform bootstrappable Quercus
Core builds.
...
To build Quercus Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Quercus Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/quercus-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/quercus-core/guix.sigs).
