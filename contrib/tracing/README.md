...
Tracing (USDT) support for Quercus Core on Linux using. For more information on
USDT support in Quercus Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `quercusd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `quercusd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Quercus Core USDT documentation] on how to list available tracepoints in your
`quercusd` binary.

[Quercus Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/quercusd
```
...
Quercus P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/quercusd
```
...
In a different terminal, starting Quercus Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/quercusd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/quercusd
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/quercusd
```
