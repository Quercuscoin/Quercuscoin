...
""" Example logging Quercus Core mempool events using the mempool:added,
...
def main(quercusd_path):
    quercusd_with_usdts = USDT(path=str(quercusd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    quercusd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    quercusd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    quercusd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    quercusd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[quercusd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/quercusd")
        exit(1)
