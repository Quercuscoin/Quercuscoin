...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing quercus-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='quercus-cli', help='quercus-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Quercus address to which the faucet should send')
...
parser.add_argument('quercus_cli_args', nargs='*', help='Arguments to pass on to quercus-cli (default: -signet)')

args = parser.parse_args()

if args.quercus_cli_args == []:
    args.quercus_cli_args = ['-signet']


def quercus_cli(rpc_command_and_params):
    argv = [args.cmd] + args.quercus_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = quercus_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = quercus_cli(['getnewaddress', 'faucet', 'bech32'])
