...
/** Call node.updateRwSetting() with Quercus 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Quercus 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/quercus/quercus/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because quercus 22.x will not try to read these.
...
//! Convert enabled/size values to quercus -prune setting.
...
//! Get pruning enabled value to show in GUI from quercus -prune setting.
...
//! Get pruning size value to show in GUI from quercus -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayQuercusUnit")) {
        settings.setValue("DisplayQuercusUnit", QVariant::fromValue(QuercusUnit::FIX));
    }
    QVariant unit = settings.value("DisplayQuercusUnit");
    if (unit.canConvert<QuercusUnit>()) {
        m_display_quercus_unit = unit.value<QuercusUnit>();
    } else {
        m_display_quercus_unit = QuercusUnit::FIX;
        settings.setValue("DisplayQuercusUnit", QVariant::fromValue(m_display_quercus_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if quercus.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_quercus_unit);
...
{
    if (new_unit.isNull() || new_unit.value<QuercusUnit>() == m_display_quercus_unit) return;
    m_display_quercus_unit = new_unit.value<QuercusUnit>();
    QSettings settings;
    settings.setValue("DisplayQuercusUnit", QVariant::fromValue(m_display_quercus_unit));
    Q_EMIT displayUnitChanged(m_display_quercus_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/quercus/quercus/pull/8273
    ...
    // (https://github.com/quercus-core/gui/issues/567).
