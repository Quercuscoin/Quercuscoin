...
#include <config/quercus-config.h>
#endif

#include <qt/quercus.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<QuercusUnit>("QuercusUnit");
#else
    qRegisterMetaType<QuercusUnit>("QuercusUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. quercus_de.qm (shortcut "de" needs to be defined in quercus.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. quercus_de_DE.qm (shortcut "de_DE" needs to be defined in quercus.qrc)
...
static const char* qt_argv = "quercus-qt";

QuercusApplication::QuercusApplication()
...
void QuercusApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the QuercusApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", QuercusGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void QuercusApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool QuercusApplication::createOptionsModel(bool resetSettings)
{
...
void QuercusApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new QuercusGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &QuercusGUI::quitRequested, this, &QuercusApplication::requestShutdown);
...
void QuercusApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void QuercusApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool QuercusApplication::baseInitialize()
{
    return node().baseInitialize();
}

void QuercusApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &QuercusApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &QuercusApplication::handleRunawayException);
    connect(this, &QuercusApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &QuercusApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void QuercusApplication::parameterSetup()
{
...
void QuercusApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void QuercusApplication::requestInitialize()
{
...
void QuercusApplication::requestShutdown()
{
...
void QuercusApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // quercus: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &QuercusGUI::handlePaymentRequest);
            connect(window, &QuercusGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void QuercusApplication::handleRunawayException(const QString &message)
{
...
void QuercusApplication::handleNonFatalException(const QString& message)
{
...
WId QuercusApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool QuercusApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", QuercusGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(quercus);
    Q_INIT_RESOURCE(quercus_locale);
    ...
    /// 6-7. Parse quercus.conf, determine network, switch to network specific
    ...
    // quercus: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
