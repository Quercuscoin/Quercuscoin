...
const std::string QuercusGUI::DEFAULT_UIPLATFORM =
...
QuercusGUI::QuercusGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &QuercusGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &QuercusGUI::showModalOverlay);
...
void QuercusGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Quercus address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and quercus: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &QuercusGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &QuercusGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &QuercusGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Quercus addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Quercus addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Quercus Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Quercus Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a quercus: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Quercus command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &QuercusGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &QuercusGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &QuercusGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &QuercusGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &QuercusGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &QuercusGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &QuercusGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &QuercusGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &QuercusGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &QuercusGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &QuercusGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &QuercusGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &QuercusGUI::showDebugWindow);
}

void QuercusGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void QuercusGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &QuercusGUI::setCurrentWalletBySelectorIndex);
...
void QuercusGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &QuercusGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &QuercusGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &QuercusGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &QuercusGUI::showProgress);
...
#ifdef ENABLE_WALLET
void QuercusGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void QuercusGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &QuercusGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &QuercusGUI::removeWallet);
...
WalletController* QuercusGUI::getWalletController()
{
    return m_wallet_controller;
}

void QuercusGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &QuercusGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &QuercusGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &QuercusGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &QuercusGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &QuercusGUI::incomingTransaction);
    connect(this, &QuercusGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void QuercusGUI::removeWallet(WalletModel* walletModel)
{
...
void QuercusGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void QuercusGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void QuercusGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void QuercusGUI::setWalletActionsEnabled(bool enabled)
{
...
void QuercusGUI::createTrayIcon()
{
...
void QuercusGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &QuercusGUI::toggleHidden);
...
void QuercusGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void QuercusGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void QuercusGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void QuercusGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void QuercusGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void QuercusGUI::openClicked()
{
...
void QuercusGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void QuercusGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void QuercusGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void QuercusGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void QuercusGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void QuercusGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void QuercusGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void QuercusGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Quercus network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void QuercusGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void QuercusGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void QuercusGUI::updateHeadersSyncProgressLabel()
{
...
void QuercusGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void QuercusGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &QuercusGUI::quitRequested);
...
void QuercusGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void QuercusGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void QuercusGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &QuercusGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &QuercusGUI::show);
                e->ignore();
            }
        }
    }
...
void QuercusGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void QuercusGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void QuercusGUI::incomingTransaction(const QString& date, QuercusUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(QuercusUnits::formatWithUnit(unit, amount, true));
...
void QuercusGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void QuercusGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool QuercusGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool QuercusGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void QuercusGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void QuercusGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void QuercusGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void QuercusGUI::updateProxyIcon()
{
    std::string ip_port;
...
void QuercusGUI::updateWindowTitle()
{
...
void QuercusGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void QuercusGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void QuercusGUI::detectShutdown()
{
...
void QuercusGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void QuercusGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(QuercusGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = QuercusGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void QuercusGUI::subscribeToCoreSignals()
{
...
void QuercusGUI::unsubscribeFromCoreSignals()
{
...
bool QuercusGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<QuercusUnit> units = QuercusUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const QuercusUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, QuercusUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const QuercusUnit u : QuercusUnits::availableUnits()) {
        menu->addAction(QuercusUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(QuercusUnit newUnits)
{
    setText(QuercusUnits::longName(newUnits));
}
