...
#include <qt/quercusaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Quercus address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new QuercusAddressEntryValidator(parent));
    widget->setCheckValidator(new QuercusAddressCheckValidator(parent));
}
...
bool parseQuercusURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no quercus: URI
    if(!uri.isValid() || uri.scheme() != QString("quercus"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!QuercusUnits::parse(QuercusUnit::FIX, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseQuercusURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseQuercusURI(uriInstance, out);
}

QString formatQuercusURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("quercus:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(QuercusUnits::format(QuercusUnit::FIX, info.amount, false, QuercusUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openQuercusConf()
{
    ...
    /* Open quercus.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Quercus.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Quercus (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Quercus (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Quercus*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "quercus.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("quercus-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a quercus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Quercus\n";
        else
            optionFile << strprintf("Name=Quercus (%s)\n", chain);
