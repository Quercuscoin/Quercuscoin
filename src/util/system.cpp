...
const char * const QUERCUS_CONF_FILENAME = "quercus.conf";
const char * const QUERCUS_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //quercus-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", QUERCUS_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Quercus
    // macOS: ~/Library/Application Support/Quercus
    // Unix-like: ~/.quercus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Quercus";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Quercus";
#else
    // Unix-like
    return pathRet / ".quercus";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", QUERCUS_CONF_FILENAME));
}
