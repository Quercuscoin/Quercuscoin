...
def parse_config_into_fix_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Quercus Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/quercus_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/quercus_config.h'), "w", encoding="utf8") as fix_config:
...
def main():
    parser = argparse.ArgumentParser(description='Quercus-core msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/quercus_config.h'), os.path.join(SOURCE_DIR, 'config/quercus-config.h'))
