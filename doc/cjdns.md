# CJDNS support in Quercus Core

It is possible to run Quercus Core over CJDNS, an encrypted IPv6 network that
...
done outside of Quercus Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Quercus network and individual nodes.
...
before it will work with your Quercus Core node. This is described in steps
...
## Run Quercus Core with CJDNS

Once you are connected to the CJDNS network, the following Quercus Core
...
When enabled, this option tells Quercus Core that it is running in an
...
IPv6 local network. This helps Quercus Core perform better address management:
...
CJDNS support was added to Quercus Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `quercus-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/quercus/quercus/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Quercus Core

There are several ways to see your CJDNS address in Quercus Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `quercus-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `quercus-cli getpeerinfo`).
