# PSBT Howto for Quercus Core

Since Quercus Core 0.17, an RPC interface exists for Partially Signed Quercus
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/quercus/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Quercus transactions that are not fully signed
...
#### Multisig with multiple Quercus Core instances
...
Quercus Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `quercus-cli` is used.
...
Later, when *V* FIX has been received on *Amulti*, and Bob and Carol want to
