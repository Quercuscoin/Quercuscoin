Quercus Core version 0.11.0 is now available from:

  <http://quercus.org/bin/quercus-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer FIX/kB fee to be
...
For example, add the following to `quercus.conf`:
...
related to the blockchain in the quercus system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Quercus Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Quercus Core will 
...
For an example script see [quercus-submittx](https://github.com/laanwj/quercus-submittx).
...
- #5334 `cf87536` libquercusconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new quercus-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only QuercusAmountField
- #5753 `0f44672` Add quercus logo to about screen
...
- #5809 `46bfbe7` Add quercus-cli man page
...
- #5849 `41259ca` contrib/init/quercusd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Quercus-Qt.app to Quercus-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for quercus-tx)
