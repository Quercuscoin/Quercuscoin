...
Below are some notes on how to build Quercus Core for Windows.

The options known to work for building Quercus Core on Windows are:
...
Note that for WSL the Quercus Core source path MUST be somewhere in the default mount file system, for
example /usr/src/quercus, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\quercus`, for example:

    make install DESTDIR=/mnt/c/workspace/quercus
