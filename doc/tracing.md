# User-space, Statically Defined Tracing (USDT) for Quercus Core

Quercus Core includes statically defined tracepoints to allow for more
...
**Note**: Quercus Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Quercus Core
...
Multiple tools can list the available tracepoints in a `quercusd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Quercus Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/quercusd
stap net        outbound_message 0x0000000000107c05 /src/quercusd
stap validation block_connected  0x00000000002fb10c /src/quercusd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Quercus Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/quercusd | grep NT_STAPSDT -A 4 -B 2
