...
TMPDIR_PREFIX = "quercus_func_test_"
...
class QuercusTestMetaClass(type):
    """Metaclass for QuercusTestFramework.

    Ensures that any attempt to register a subclass of `QuercusTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'QuercusTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("QuercusTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("QuercusTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class QuercusTestFramework(metaclass=QuercusTestMetaClass):
    """Base class for a quercus test script.

    Individual quercus test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave quercusds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop quercusds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use quercus-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_quercusd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "quercusd" + config["environment"]["EXEEXT"],
        )
        fname_quercuscli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "quercus-cli" + config["environment"]["EXEEXT"],
        )
        fname_quercusutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "quercus-util" + config["environment"]["EXEEXT"],
        )
        self.options.quercusd = os.getenv("QUERCUSD", default=fname_quercusd)
        self.options.quercuscli = os.getenv("QUERCUSCLI", default=fname_quercuscli)
        self.options.quercusutil = os.getenv("QUERCUSUTIL", default=fname_quercusutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: quercusds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("QuercusRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'quercusd', self.options.quercusd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'quercus-cli', self.options.quercuscli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a quercusd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple quercusd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as quercusd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    quercusd=self.options.quercusd,
                    quercus_cli=self.options.quercuscli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in quercus.conf
    ...
    def skip_if_no_quercusd_tracepoints(self):
        """Skip the running test if quercusd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("quercusd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_quercusd_zmq(self):
        """Skip the running test if quercusd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("quercusd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if quercus-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("quercus-wallet has not been compiled")

    def skip_if_no_quercus_util(self):
        """Skip the running test if quercus-util has not been compiled."""
        if not self.is_quercus_util_compiled():
            raise SkipTest("quercus-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if quercus-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("quercus-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether quercus-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether quercus-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_quercus_util_compiled(self):
        """Checks whether quercus-util was compiled."""
        return self.config["components"].getboolean("ENABLE_QUERCUS_UTIL")
