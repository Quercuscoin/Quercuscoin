...
# TestP2PConn: A peer we use to send messages to quercusd, and store responses.
...
class CompactBlocksTest(QuercusTestFramework):
    ...
    # This test actually causes quercusd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that quercusd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for quercusd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # quercusd will not send blocktxn responses for blocks whose height is
