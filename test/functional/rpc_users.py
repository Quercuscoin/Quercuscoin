...
class HTTPBasicsTest(QuercusTestFramework):
    ...
    def setup_chain(self):
        super().setup_chain()
        #Append rpcauth to quercus.conf before initialization
        ...
        with open(os.path.join(get_datadir_path(self.options.tmpdir, 0), "quercus.conf"), 'a', encoding='utf8') as f:
            f.write(rpcauth + "\n")
            f.write(rpcauth2 + "\n")
            f.write(rpcauth3 + "\n")
        with open(os.path.join(get_datadir_path(self.options.tmpdir, 1), "quercus.conf"), 'a', encoding='utf8') as f:
            f.write("rpcuser={}\n".format(self.rpcuser))
